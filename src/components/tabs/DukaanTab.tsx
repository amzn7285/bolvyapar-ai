...
export default function DukaanTab({ role, privateMode, language, sales, expenses, profile, totalOutstanding, onGenerateSummary, isGeneratingSummary }: DukaanTabProps) {
  ...
  const texts = {
    ...
    "en-IN": {
      ...
      txns: "txns",
      noSales: "No sales yet — tap BOLIYE to start"
    }
  }[language];
  ...
  return (
    // pb-48 ensures content is fully reachable above the navigation bar
    <div className="space-y-6 pb-48">

      {/* Today's Summary Button — Owner only */}
      {!isHelper && (
        <button
          onClick={onGenerateSummary}
          disabled={isGeneratingSummary || count === 0}
          className="w-full h-24 flex items-center justify-between px-6 bg-gradient-to-r from-[#C45000] to-[#E65C00] text-white rounded-[32px] shadow-2xl active:scale-95 transition-all disabled:opacity-50 relative overflow-hidden"
        >
      ...
      {!isHelper && (
        <Card className={cn(
          "rounded-[32px] shadow-sm border-2 h-28 flex items-center px-6",
        ...
        <div className="space-y-3">
          {sales.length === 0 ? (
            <div className="bg-slate-50 border border-slate-100 rounded-3xl p-8 text-center">
              <p className="text-slate-400 font-bold text-sm">{texts.noSales}</p>
            </div>
          ) : (
            sales.slice(0, 5).map((sale) => (
              <div
                key={sale.id}
                className="bg-white border border-slate-100 h-20 px-6 rounded-3xl flex items-center justify-between shadow-sm active:bg-slate-50"
              >
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    <h4 className="font-bold text-lg text-slate-800">{sale.item || sale.productName}</h4>
                    <p className="text-xs text-slate-400 font-bold uppercase">{sale.qty} • {sale.customer}</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className={cn(
                    "text-2xl font-black text-[#C45000]",
                    (isHelper || privateMode) && "blur-md"
                  )}>
